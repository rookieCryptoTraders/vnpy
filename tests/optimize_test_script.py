...
logger = logger.bind(gateway_name="TEST")
...
def generate_fake_ohlcv_wide_dict(
    start_date: datetime, end_date: datetime, interval: str, vt_symbols: list[str]
    ...
    random_shocks = np.random.multivariate_normal(
        np.zeros(n_symbols), cov_matrix, n_dates
    )
    ...
    open_prices = np.vstack(
        [
            close_prices[0] * (1 + (np.random.rand(n_symbols) - 0.5) * 0.01),
            close_prices[:-1],
        ]
    )
    high_prices = (
        np.maximum(open_prices, close_prices)
        + np.random.uniform(0, 0.01, (n_dates, n_symbols)) * close_prices
    )
    low_prices = (
    ...
    ohlcv_dict = {
        ohlcv_type: flat_df.pivot(
            values=ohlcv_type, index="datetime", columns="symbol"
        ).sort("datetime")
...
def main():
    ...
    if best_params:
        ...
        logger.info(
            f"Best Sharpe Ratio (on training set): {search_results.get('best_score', 'N/A'):.4f}"
        )
        logger.info(f"Final report for best parameters generated at: {report_path}")

        print("\n--- Grid Search Results (Top 5) ---")
        sorted_results = sorted(
            zip(search_results["params"], search_results["scores"], strict=False),
        ...
        logger.warning(
            "Could not find suitable parameters or an error occurred during the process."
        )
