...
class MULTFactor(FactorTemplate):
    """
    Calculates the MULT indicator.
    """
    ...
    def __init__(self, setting: dict | None = None, vt_symbols: list[str] | None = None, **kwargs):
        """
        Initializes the MULTFactor.
        Required parameters: real0, real1
        """
        super().__init__(setting, **kwargs)
        self.vt_symbols: list[str] = vt_symbols if vt_symbols else []

        # --- Parameter Validation and Assignment ---
        if not hasattr(self.params, "real0"): raise ValueError(f"{self.factor_key} requires a 'real0' parameter.")
        try: self.real0 = float(self.params.real0)
        except (ValueError, TypeError): self.real0 = int(self.params.real0)
        if not hasattr(self.params, "real1"): raise ValueError(f"{self.factor_key} requires a 'real1' parameter.")
        try: self.real1 = float(self.params.real1)
        except (ValueError, TypeError): self.real1 = int(self.params.real1)
    ...
    def calculate(self, input_data: Dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        kwargs = { "real0": self.real0, "real1": self.real1 }
        expressions_to_run = []

        for symbol in symbol_columns:
            symbol_inputs = {  }

            result_expr = plta.mult(**symbol_inputs, **kwargs)
