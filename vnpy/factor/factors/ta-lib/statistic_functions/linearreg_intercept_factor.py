...
class LINEARREG_INTERCEPTFactor(FactorTemplate):
    """
    Calculates the LINEARREG_INTERCEPT indicator.
    """

    author = "Auto-Generated by FactorGenerator"
    factor_name = "LINEARREG_INTERCEPT"

    def __init__(self, setting: dict | None = None, vt_symbols: list[str] | None = None, **kwargs):
        """
        Initializes the LINEARREG_INTERCEPTFactor.
        Required parameters: price, timeperiod
        """
    ...
    def calculate(self, input_data: Dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        kwargs = { "price": self.price, "timeperiod": self.timeperiod }
        expressions_to_run = []

        for symbol in symbol_columns:
            symbol_inputs = {  }

            result_expr = plta.linearreg_intercept(**symbol_inputs, **kwargs)
