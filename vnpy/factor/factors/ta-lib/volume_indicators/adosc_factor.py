...
class ADOSCFactor(FactorTemplate):
    """
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        if input_data.get("volume") is None or input_data["volume"].is_empty(): return pl.DataFrame(data={}, schema=self.get_output_schema())
        ...
        for symbol in symbol_columns:
            symbol_inputs = { "high": pl.col(symbol), "low": pl.col(symbol), "close": pl.col(symbol), "volume": pl.col(symbol) }

            result_expr = plta.adosc(**symbol_inputs, **kwargs)
        ...
        for col_name, df_wide in input_data.items():
            if col_name != "close" and col_name in {'high', 'close', 'volume', 'low'}:
