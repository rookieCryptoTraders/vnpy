...
class OBVFactor(FactorTemplate):
    """
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        for symbol in symbol_columns:
            symbol_inputs = { "close": pl.col(symbol), "volume": pl.col(symbol) }

            result_expr = plta.obv(**symbol_inputs, **kwargs)
        ...
        for col_name, df_wide in input_data.items():
            if col_name != "close" and col_name in {'volume', 'close'}:
