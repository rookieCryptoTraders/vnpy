...
class CDLEVENINGDOJISTAR(FactorTemplate):
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        if input_data.get("open") is None: return pl.DataFrame(data={}, schema=self.get_output_schema())
        ...
        for symbol in symbol_columns:
            ...
            symbol_inputs = { "open": input_data["open"][symbol], "high": input_data["high"][symbol], "low": input_data["low"][symbol], "close": input_data["close"][symbol] }

            # Execute the calculation immediately for this single symbol
            result_series_or_struct = plta.cdleveningdojistar(**symbol_inputs, **kwargs)
