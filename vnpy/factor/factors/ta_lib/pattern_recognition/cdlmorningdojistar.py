...
class CDLMORNINGDOJISTAR(FactorTemplate):
    """
    Calculates the CDLMORNINGDOJISTAR indicator.
    """

    author = "Auto-Generated by FactorGenerator"
    factor_name = "cdlmorningdojistar"

    def __init__(self, setting: dict | None = None, **kwargs):
        """
        Initializes the CDLMORNINGDOJISTAR.
        Required parameters: penetration
        """
        super().__init__(setting, **kwargs)

        # --- Parameter Validation and Assignment ---
        if not hasattr(self.params, "penetration"): raise ValueError(f"{self.factor_key} requires a 'penetration' parameter.")
        try: self.penetration = int(self.params.penetration)
        except (ValueError, TypeError): self.penetration = float(self.params.penetration)
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        if input_data["open"].is_empty(): return pl.DataFrame(data={}, schema=self.get_output_schema())
        ...
        kwargs = { "penetration": self.penetration }
        output_series_list = []

        for symbol in symbol_columns:
            # 1. Create a temporary DataFrame with this symbol's required inputs
            temp_df_inputs = { "open": input_data["open"][symbol], "high": input_data["high"][symbol], "low": input_data["low"][symbol], "close": input_data["close"][symbol] }
            input_df = pl.DataFrame(temp_df_inputs)

            # 2. Define the calculation expression using pl.col()
            calc_expression = plta.cdlmorningdojistar(open=pl.col("open"), high=pl.col("high"), low=pl.col("low"), close=pl.col("close"), **kwargs).alias("result")
