...
class STOCH_SLOWK(FactorTemplate):
    ...
    def __init__(self, setting: dict | None = None, **kwargs):
        ...
        try: self.slowk_period = int(self.params.slowk_period)
        except (ValueError, TypeError): self.slowk_period = float(self.params.slowk_period)
        if not hasattr(self.params, "slowk_matype"): raise ValueError(f"{self.factor_key} requires a 'slowk_matype' parameter.")
        try: self.slowk_matype = int(self.params.slowk_matype)
        except (ValueError, TypeError): self.slowk_matype = float(self.params.slowk_matype)
        if not hasattr(self.params, "slowd_period"): raise ValueError(f"{self.factor_key} requires a 'slowd_period' parameter.")
        try: self.slowd_period = int(self.params.slowd_period)
        except (ValueError, TypeError): self.slowd_period = float(self.params.slowd_period)
        if not hasattr(self.params, "slowd_matype"): raise ValueError(f"{self.factor_key} requires a 'slowd_matype' parameter.")
        try: self.slowd_matype = int(self.params.slowd_matype)
        except (ValueError, TypeError): self.slowd_matype = float(self.params.slowd_matype)
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        for symbol in symbol_columns:
            ...
            calc_expression = plta.stoch(high=pl.col("high"), low=pl.col("low"), close=pl.col("close"), **kwargs).alias("result")
