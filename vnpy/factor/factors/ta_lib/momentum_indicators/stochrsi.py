...
class STOCHRSI(FactorTemplate):
    """
    Calculates the STOCHRSI indicator.
    """

    author = "Auto-Generated by FactorGenerator"
    factor_name = "stochrsi"

    def __init__(self, setting: dict | None = None, **kwargs):
        """
        Initializes the STOCHRSI.
        Required parameters: timeperiod, fastk_period, fastd_period, fastd_matype
        """
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        kwargs = { "timeperiod": self.timeperiod, "fastk_period": self.fastk_period, "fastd_period": self.fastd_period, "fastd_matype": self.fastd_matype }
        output_series_list = []

        for symbol in symbol_columns:
            # 1. Create a temporary DataFrame with this symbol's required inputs
            temp_df_inputs = { "close": input_data["close"][symbol] }
            input_df = pl.DataFrame(temp_df_inputs)

            # 2. Define the calculation expression using pl.col()
            calc_expression = plta.stochrsi(close=pl.col("close"), **kwargs).alias("result")
