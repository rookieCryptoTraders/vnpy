...
class ADOSC(FactorTemplate):
    """
    Calculates the ADOSC indicator.
    """
    ...
    def __init__(self, setting: dict | None = None, **kwargs):
        """
        Initializes the ADOSC.
        Required parameters: fastperiod, slowperiod
        """
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        if input_data["volume"].is_empty(): return pl.DataFrame(data={}, schema=self.get_output_schema())
        ...
        kwargs = { "fastperiod": self.fastperiod, "slowperiod": self.slowperiod }
        output_series_list = []

        for symbol in symbol_columns:
            # 1. Create a temporary DataFrame with this symbol's required inputs
            temp_df_inputs = { "high": input_data["high"][symbol], "low": input_data["low"][symbol], "close": input_data["close"][symbol], "volume": input_data["volume"][symbol] }
            input_df = pl.DataFrame(temp_df_inputs)

            # 2. Define the calculation expression using pl.col()
            calc_expression = plta.adosc(high=pl.col("high"), low=pl.col("low"), close=pl.col("close"), volume=pl.col("volume"), **kwargs).alias("result")
