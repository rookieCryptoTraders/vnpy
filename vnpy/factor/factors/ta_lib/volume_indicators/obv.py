...
class OBV(FactorTemplate):
    """
    Calculates the OBV indicator.
    """
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        for symbol in symbol_columns:
            ...
            temp_df_inputs = { "close": input_data["close"][symbol], "volume": input_data["volume"][symbol] }
            input_df = pl.DataFrame(temp_df_inputs)

            # 2. Define the calculation expression using pl.col()
            calc_expression = plta.obv(close=pl.col("close"), volume=pl.col("volume"), **kwargs).alias("result")
