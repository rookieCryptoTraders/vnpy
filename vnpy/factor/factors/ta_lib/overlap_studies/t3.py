...
class T3(FactorTemplate):
    """
    Calculates the T3 indicator.
    """
    ...
    def __init__(self, setting: dict | None = None, **kwargs):
        """
        Initializes the T3.
        Required parameters: timeperiod, vfactor
        """
        ...
        if not hasattr(self.params, "vfactor"): raise ValueError(f"{self.factor_key} requires a 'vfactor' parameter.")
        try: self.vfactor = int(self.params.vfactor)
        except (ValueError, TypeError): self.vfactor = float(self.params.vfactor)
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        kwargs = { "timeperiod": self.timeperiod, "vfactor": self.vfactor }
        output_series_list = []
        ...
        for symbol in symbol_columns:
            ...
            calc_expression = plta.t3(real=pl.col("real"), **kwargs).alias("result")
