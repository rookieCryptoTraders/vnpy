...
class MAVP(FactorTemplate):
    ...
    def __init__(self, setting: dict | None = None, **kwargs):
        ...
        try: self.periods = int(self.params.periods)
        except (ValueError, TypeError): self.periods = float(self.params.periods)
        if not hasattr(self.params, "minperiod"): raise ValueError(f"{self.factor_key} requires a 'minperiod' parameter.")
        try: self.minperiod = int(self.params.minperiod)
        except (ValueError, TypeError): self.minperiod = float(self.params.minperiod)
        if not hasattr(self.params, "maxperiod"): raise ValueError(f"{self.factor_key} requires a 'maxperiod' parameter.")
        try: self.maxperiod = int(self.params.maxperiod)
        except (ValueError, TypeError): self.maxperiod = float(self.params.maxperiod)
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        for symbol in symbol_columns:
            ...
            calc_expression = plta.mavp(real=pl.col("real"), **kwargs).alias("result")
