...
class SAR(FactorTemplate):
    ...
    def __init__(self, setting: dict | None = None, **kwargs):
        ...
        try: self.acceleration = int(self.params.acceleration)
        except (ValueError, TypeError): self.acceleration = float(self.params.acceleration)
        if not hasattr(self.params, "maximum"): raise ValueError(f"{self.factor_key} requires a 'maximum' parameter.")
        try: self.maximum = int(self.params.maximum)
        except (ValueError, TypeError): self.maximum = float(self.params.maximum)
    ...
    def calculate(self, input_data: dict[str, pl.DataFrame], memory: FactorMemory) -> pl.DataFrame:
        ...
        for symbol in symbol_columns:
            ...
            calc_expression = plta.sar(high=pl.col("high"), low=pl.col("low"), **kwargs).alias("result")
