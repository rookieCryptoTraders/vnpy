...
class FactorAnalyser:
    ...
    def __init__(self, vt_symbols: list = [], output_data_dir_for_reports: str | None = None):
        ...
        self.long_short_stats: dict[str, float] | None = None
        self.performance_metrics: dict[str, Any] | None = None

        self._write_log(
            f"FactorAnalyser initialized. Report dir: {self.output_data_dir}",
            level=INFO,
        )
    ...
    def _prepare_output_directory(self) -> None:
        try:
            ...
            self._write_log(
                f"Report directory ensured at: {self.output_data_dir}", level=INFO
            )
        except OSError as e:
            self._write_log(
                f"Error creating report directory {self.output_data_dir}: {e}. Reports may fail to save.",
                level=ERROR,
            )
    ...
    def perform_quantile_analysis(
        ...
        self._write_log(
            f"Performing quantile analysis: {num_quantiles} quantiles, using {returns_look_ahead_period}-period forward returns.",
            level=INFO,
        )
        if factor_data_df is None or factor_data_df.is_empty():
            self._write_log("Factor data empty for quantile analysis.", level=ERROR)
            return False
        if self.symbol_returns_df is None or self.symbol_returns_df.is_empty():
            self._write_log(
                "Symbol returns data empty for quantile analysis.", level=ERROR
            )
            return False
        ...
        symbols_for_analysis = [
            sym for sym in symbols_in_factor if sym in self.vt_symbols
        ]

        if not symbols_for_analysis:
            self._write_log(
                "No common symbols between factor data and returns data for quantile analysis.",
                level=ERROR,
            )
            return False
        if returns_look_ahead_period != 1:
            self._write_log(
                f"Warning: Quantile analysis with look_ahead={returns_look_ahead_period}, ensure symbol_returns_df is prepared accordingly.",
                level=WARNING,
            )
        ...
        merged_df = factor_long_df.join(
            returns_long_df, on=[self.factor_datetime_col, "symbol"], how="inner"
        ).drop_nulls(subset=["forward_return"])
        if merged_df.is_empty():
            self._write_log(
                "No valid data after merging for quantile analysis.", level=ERROR
            )
            return False
        ...
        quantiled_df = merged_df.with_columns(
            (
                ...
                * num_quantiles
                / pl.col("symbol").count().over(self.factor_datetime_col)
            )
            .floor()
            .cast(pl.Int8)
            + (1).alias("quantile")
        )
        if quantiled_df.is_empty():
            self._write_log("Quantiled DataFrame empty.", level=ERROR)
            return False

        quantile_returns_by_time_df = (
            quantiled_df.group_by([self.factor_datetime_col, "quantile"])
            .agg(pl.col("forward_return").mean().alias("mean_quantile_return"))
            .sort([self.factor_datetime_col, "quantile"])
        )
        average_quantile_returns_overall_df = (
            quantile_returns_by_time_df.group_by("quantile")
            .agg(
                pl.col("mean_quantile_return").mean().alias("average_return"),
                pl.col("mean_quantile_return").std().alias("std_dev_return"),
        ...
        self._write_log(
            f"Quantile analysis completed. Overall:\n{average_quantile_returns_overall_df if not average_quantile_returns_overall_df.is_empty() else 'Empty'}",
            level=INFO,
        )
        return True
    ...
    def perform_long_short_analysis(
        ...
        self._write_log(
            f"Performing L/S analysis. Long > {long_percentile_threshold * 100:.0f}th, Short <= {short_percentile_threshold * 100:.0f}th.",
            level=INFO,
        )
        if factor_data_df is None or factor_data_df.is_empty():
            self._write_log("Factor data empty for L/S analysis.", level=ERROR)
            return False
        if self.symbol_returns_df is None or self.symbol_returns_df.is_empty():
            self._write_log("Symbol returns empty for L/S analysis.", level=ERROR)
            return False
        ...
        if not symbols_for_analysis:
            self._write_log("No common symbols for L/S analysis.", level=ERROR)
            return False
        if returns_look_ahead_period != 1:
            self._write_log(
                f"Warning: L/S analysis with look_ahead={returns_look_ahead_period}, ensure symbol_returns_df is prepared accordingly.",
                level=WARNING,
            )
        ...
        if merged_df.is_empty():
            self._write_log(
                "No valid data after merging for L/S analysis.", level=ERROR
            )
            return False
        ...
        ranked_df = merged_df.with_columns(
            [
                (
                    ...
                    .over(self.factor_datetime_col)
                    / pl.col("symbol").count().over(self.factor_datetime_col)
                ).alias("rank_percentile")
            ]
        )
        portfolio_df = ranked_df.with_columns(
            [
                (pl.col("rank_percentile") > long_percentile_threshold).alias(
                    "long_leg"
                ),
                (pl.col("rank_percentile") <= short_percentile_threshold).alias(
                    "short_leg"
                ),
            ]
        )
        long_ret = (
            portfolio_df.filter(pl.col("long_leg"))
            .group_by(self.factor_datetime_col)
            .agg(pl.col("forward_return").mean().alias("mean_long_return"))
        )
        short_ret = (
            portfolio_df.filter(pl.col("short_leg"))
            .group_by(self.factor_datetime_col)
            .agg(pl.col("forward_return").mean().alias("mean_short_return"))
        )
        ls_calc_df = long_ret.join(
            short_ret, on=self.factor_datetime_col, how="outer"
        ).sort(self.factor_datetime_col)
        ls_calc_df = ls_calc_df.with_columns(
            [
                pl.col("mean_long_return").fill_null(0.0),
                pl.col("mean_short_return").fill_null(0.0),
            ]
        ).with_columns(
            (pl.col("mean_long_return") - pl.col("mean_short_return")).alias(
                "ls_portfolio_return"
            )
        )
        self.long_short_portfolio_returns_df = ls_calc_df.select(
            [
        ...
        port_rets_ttest = self.long_short_portfolio_returns_df[
            "ls_portfolio_return"
        ].drop_nulls()
        if port_rets_ttest.len() >= 2:
            ...
            if mean_ret is None:
                ...
                t_stat, p_val = stats.ttest_1samp(
                    port_rets_ttest.to_numpy(), 0, nan_policy="omit"
                )
                self.long_short_stats = {
                    "t_statistic": t_stat,
                    "p_value": p_val,
            ...
            self.long_short_stats = {
                "t_statistic": float("nan"),
                "p_value": float("nan"),
                "mean_return": port_rets_ttest.mean()
                if not port_rets_ttest.is_empty()
                else float("nan"),
            }
            self._write_log("Not enough L/S returns for t-test.", level=WARNING)
        return True

    def calculate_performance_metrics(self) -> bool:
        self._write_log(
            "Calculating L/S portfolio performance metrics (quantstats)...", level=INFO
        )
        if (
            self.long_short_portfolio_returns_df is None
            or self.long_short_portfolio_returns_df.is_empty()
        ):
            self._write_log("L/S portfolio returns empty for metrics.", level=ERROR)
            return False
        try:
            returns_pl_filled = self.long_short_portfolio_returns_df.sort(
                self.factor_datetime_col
            ).with_columns(pl.col("ls_portfolio_return").fill_null(0.0))
            portfolio_pd_df = (
                returns_pl_filled.select(
                    [self.factor_datetime_col, "ls_portfolio_return"]
                )
                .to_pandas()
                .set_index(self.factor_datetime_col)
            )
            returns_series_pd = portfolio_pd_df["ls_portfolio_return"].astype(float)
            if returns_series_pd.empty:
                self._write_log("Pandas returns series empty for metrics.", level=ERROR)
                return False
            if returns_series_pd.isnull().any() or np.isinf(returns_series_pd).any():
                returns_series_pd = returns_series_pd.fillna(0.0).replace(
                    [np.inf, -np.inf], 0.0
                )
        except Exception as e:
            self._write_log(f"Error preparing data for quantstats: {e}", level=ERROR)
            return False

        try:
            metrics = {
                "sharpe": qs.stats.sharpe(returns_series_pd, smart=True),
                "max_drawdown": qs.stats.max_drawdown(returns_series_pd),
                "cumulative_returns_total": qs.stats.comp(returns_series_pd).iloc[-1],
                "cagr": qs.stats.cagr(returns_series_pd),
                "volatility_annualized": qs.stats.volatility(
                    returns_series_pd, annualize=True
                ),
                ...
                "sortino": qs.stats.sortino(returns_series_pd, smart=True),
                "calmar": qs.stats.calmar(returns_series_pd),
                "skew": qs.stats.skew(returns_series_pd),
                "kurtosis": qs.stats.kurtosis(returns_series_pd),
                "value_at_risk_daily": qs.stats.value_at_risk(returns_series_pd),
                "expected_shortfall_daily": qs.stats.conditional_value_at_risk(
                    returns_series_pd
                ),
                "profit_factor": qs.stats.profit_factor(returns_series_pd),
            ...
            self.performance_metrics = {
                k: (
                    None
                    if isinstance(v, float|np.generic)
                    and (np.isnan(v) or np.isinf(v))
                    else (v.to_dict() if isinstance(v, pd.Series) else v)
                )
                for k, v in metrics.items()
            }
            self._write_log(
                f"Performance metrics calculated. Sharpe: {self.performance_metrics.get('sharpe', 'N/A'):.3f}",
                level=INFO,
            )
            return True
        except Exception as e:
    ...
    def generate_report_data(
        ...
        if self.quantile_analysis_results:
            ...
            report_data["analysis_results"]["quantile_analysis"] = {
                "overall_average_returns_per_quantile": qa_overall.to_dicts()
                if qa_overall is not None and not qa_overall.is_empty()
                else None,
            }
        if self.long_short_stats:
            report_data["analysis_results"]["long_short_portfolio_statistics"] = (
                self.long_short_stats
            )
        ...
        if not report_data["analysis_results"]:
            self._write_log(
                "No analysis results were available for the report.", level=WARNING
            )
        return report_data
    ...
    def save_report(
        ...
        if not report_data:
            self._write_log("Report data empty. Nothing to save.", level=WARNING)
            return None
        if not self.output_data_dir.exists():
            try:
                ...
                self._write_log(
                    f"Error creating report dir {self.output_data_dir}: {e}",
                    level=ERROR,
                )
                return None
        ...
        try:
            ...
            self._write_log(f"Error saving report to {filepath}: {e}", level=ERROR)
            return None
    ...
    def run_analysis_and_report(
        ...
        """
        self._write_log(
            f"Starting analysis for factor: {factor_instance.factor_key}", level=INFO
        )

        if factor_data_df.is_empty():
            self._write_log(
                "Input factor_data_df is empty. Cannot perform analysis.", level=ERROR
            )
            return None
        if market_close_prices_df.is_empty():
            self._write_log(
                "Input market_close_prices_df is empty. Cannot perform analysis.",
                level=ERROR,
            )
            return None
        ...
        if not self.prepare_symbol_returns(market_close_prices_df):
            ...
            self.perform_quantile_analysis(
                factor_data_df, num_quantiles, returns_look_ahead_period
            )
            self.perform_long_short_analysis(
                factor_data_df,
                returns_look_ahead_period,
                long_percentile_threshold,
                short_percentile_threshold,
            )
            if (
                self.long_short_portfolio_returns_df is not None
                and not self.long_short_portfolio_returns_df.is_empty()
            ):
                self.calculate_performance_metrics()
            else:
                self._write_log(
                    "Skipping performance metrics: L/S portfolio returns unavailable.",
                    INFO,
                )
        ...
        self._write_log(
            f"Analysis and reporting for {factor_instance.factor_key} completed.",
            level=INFO,
        )
        return saved_path
